-- Adds each approved name's etymology, for the info drawer on Your Names. See
-- ticket [096].
--
-- Built by get_given_name_etymology (047), the same function candidates use. A
-- custom name has no given_name_id, so its etymology is null.
--
-- Dropped first because the return type gains a column, which CREATE OR REPLACE
-- cannot do.

DROP FUNCTION IF EXISTS get_approved_given_names(INT);
...
BEGIN
  ...
  SELECT
    ...
    get_given_name_etymology(gcnb.given_name_id) AS out_etymology
